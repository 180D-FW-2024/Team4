...
class VLight:
    ...
    def calculate_v(self):
        ...
        """
        # Convert img_hsv to float32 and normalize to [0, 1]
        img_hsv_float32 = self.img_hsv.astype(np.float32) / 255.0

        # Compute mean of the V-channel
        v_mean = np.mean(img_hsv_float32[:, :, 2])
        
        # Return the adjusted value
        return 1 - min(v_mean / 0.35, 1)
    ...
    def apply_kernel(self, v=0.5, color=False, lut=True):
        """
